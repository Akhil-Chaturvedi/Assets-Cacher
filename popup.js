document.addEventListener('DOMContentLoaded', () => {
  ...
  enabledSwitch.addEventListener('change', () => {
    ...
    chrome.runtime.sendMessage({
      ...
      // Re-fetch state to confirm the toggle took effect
      chrome.runtime.sendMessage(
        { type: 'getState', hostname: currentHostname },
        (state) => {
          if (chrome.runtime.lastError) {
            console.error("State refresh Error:", chrome.runtime.lastError.message);
          } else {
            updateUI(state);
          }
        }
  ...
  purgeButton.addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ type: 'purgeSiteCache', hostname: currentHostname }, () => {
      if (chrome.runtime.lastError) {
        console.error("Purge Error:", chrome.runtime.lastError.message);
        container.classList.remove('loading');
        return;
      }
      chrome.runtime.sendMessage({ type: 'getState', hostname: currentHostname }, (state) => {
